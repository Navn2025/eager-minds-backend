...
function normalizeDifficulty(input: string | undefined): QuizDifficulty {
  if (input === "easy" || input === "hard") return input;
  return "medium";
}

function hasVisualDependency(text: string): boolean {
  return /\b(image|picture|diagram|figure|chart|graph|shape|matrix|visual|shown|above)\b/i.test(
    text,
  );
}
...
function isPlaceholderChoice(option: string): boolean {
...
function extractQuestionsFromParsed(parsed: unknown): unknown {
  ...
  if (obj.quiz && typeof obj.quiz === "object") {
    const quizObj = obj.quiz as Record<string, unknown>;
    if (Array.isArray(quizObj.questions)) return quizObj.questions;
  }

  if (obj.data && typeof obj.data === "object") {
    const dataObj = obj.data as Record<string, unknown>;
    if (Array.isArray(dataObj.questions)) return dataObj.questions;
  }
...
function getOptionsFromQuestion(item: Record<string, unknown>): string[] {
  if (Array.isArray(item.options)) {
    return item.options.filter((o): o is string => typeof o === "string");
  }
...
function normalizeAnswerIndex(
  ...
  const answerLike =
    item.answer ?? item.correctAnswer ?? item.correctOption ?? item.answerIndex;
...
async function requestGroqQuizContent(
  apiKey: string,
  model: string,
  userPrompt: string,
  temperature: number,
    ...
    messages: [
      {
        role: "system",
        content:
          "You are an exam writer for 11+ GL Assessment style multiple-choice questions. Generate text-only questions and return only JSON.",
      },
      {
        role: "user",
        content: userPrompt,
  ...
  const completion = (await groqRes.json()) as GroqResponse;
...
function generateQuizNumber(): string {
  const now = new Date();
...
function getQuizBadge(percentage: number): string {
  if (percentage >= 90) return "Quiz Champion";
  if (percentage >= 75) return "Gold Scholar";
  if (percentage >= 60) return "Silver Solver";
  if (percentage >= 40) return "Bronze Builder";
  return "Rising Learner";
}

function calculateQuizRewards(
  score: number,
  ...
  const bonusXp =
    percentage >= 90 ? 30 : percentage >= 75 ? 20 : percentage >= 60 ? 10 : 0;
...
function getXpBadge(totalXp: number): string {
...
function getFreePaperUnlocks(totalXp: number): number {
  return FREE_PAPER_XP_THRESHOLDS.filter((threshold) => totalXp >= threshold)
    .length;
}

function getNextFreePaperXp(totalXp: number): number | null {
  return (
    FREE_PAPER_XP_THRESHOLDS.find((threshold) => threshold > totalXp) ?? null
  );
}

function computeDayStreak(dateStrings: string[]): number {
...
export async function generateQuiz(
  ...
  try {
    ...
    const safeTitle =
      typeof title === "string" ? title.trim() : "Practice Quiz";
    const safeCategory =
    ...
    const safeDifficulty = normalizeDifficulty(
      typeof difficulty === "string" ? difficulty.toLowerCase() : undefined,
    );
    const safeQuestionCount =
      typeof requestedCount === "number" ? Math.floor(requestedCount) : 10;
    const fixedQuestionCount = 10;

    const unsupportedVisualTopicPattern =
      /\b(matrix|matrices|non-?verbal|pattern(?:s)?|shape(?:s)?|figure(?:s)?|diagram(?:s)?|visual(?:s)?)\b/i;

    if (
      unsupportedVisualTopicPattern.test(safeTopic) ||
      unsupportedVisualTopicPattern.test(safeCategory)
    ) {
      res.status(400).json({
        message:
          "This topic needs visual/image generation, which is not supported yet. Please choose a text-based topic.",
      });
      return;
    }
    ...
    for (let attempt = 1; attempt <= 2; attempt += 1) {
      try {
        ...
        const content = await requestGroqQuizContent(
          apiKey,
          model,
          attemptPrompt,
          attempt === 1 ? 0.4 : 0.2,
        ...
        questions = sanitizeQuestions(rawQuestions, safeDifficulty).slice(
          0,
          fixedQuestionCount,
        );
        ...
        const message =
          error instanceof Error ? error.message : "generation_error";
    ...
    const persistedQuizPayload = JSON.parse(
      JSON.stringify(quiz),
...
export async function completeQuiz(
  ...
  try {
    ...
    const safeQuizNumber =
      typeof quizNumber === "string" ? quizNumber.trim() : "";
    if (!safeQuizNumber) {
    ...
    const safeTotal =
      typeof totalQuestions === "number" && Number.isFinite(totalQuestions)
        ...
        : 10;
    const safeSelectedAnswers = normalizeSelectedAnswers(
      selectedAnswers,
      safeTotal,
    );
    const safePercentage = Math.round((safeScore / safeTotal) * 100);
    const safeTitle =
      typeof title === "string" && title.trim() ? title.trim() : "Saved Quiz";
    const safeCategory =
    ...
    const safeTopic =
      typeof topic === "string" && topic.trim()
        ...
        : "Mixed Practice";
    const safeType =
      typeof type === "string" && type.trim() ? type.trim() : "Practice";
    const safeAttemptDifficulty =
      typeof difficulty === "string" && difficulty.trim()
        ? difficulty.trim().toLowerCase()
        : "medium";
    const safeAttemptPayload: Prisma.InputJsonValue = {
    ...
    const streakDays = computeDayStreak(
      recentQuizCompletions.map((c) => c.completedAt.toISOString()),
    );
...
export async function getQuizAttempts(
  ...
  try {
    ...
    res.json({ attempts: attempts as QuizAttemptSummary[] });
  } catch (error) {
