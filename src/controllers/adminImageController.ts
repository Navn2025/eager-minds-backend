...
import {
  NAVBAR_MENU_SECTIONS,
  readNavbarMenuImages,
  writeNavbarMenuImages,
} from "../lib/navbarHomeImagesStore.js";
...
const NAVBAR_MENU_SECTION_IDS = new Set(
  NAVBAR_MENU_SECTIONS.map((section) => section.id),
);
...
function isNavbarMenuSectionId(value: string): boolean {
...
async function maybeDeleteCloudinaryAsset(url?: string | null): Promise<void> {
  if (!url) return;
...
export async function getSiteImages(
  ...
  try {
    const [blog, competitions, activities, magazines, artsCraft, navbarMenu] =
      await Promise.all([
...
export async function uploadSiteImage(
  ...
  try {
    const file = req.file;
    const { entityType, entityId, index, sectionId } = req.body as {
      entityType?: string;
      entityId?: string;
      index?: string;
      sectionId?: string;
    ...
    const upload = await uploadFileToCloud(
      file,
      `eager-minds/site-images/${entityType}`,
    );
    ...
    switch (entityType) {
      ...
      case "navbarMenu": {
        const resolvedSectionId = sectionId as string;
        ...
        const nextImages: [string | null, string | null] = [
          existingImages[0],
          existingImages[1],
        ];
        ...
        const section = NAVBAR_MENU_SECTIONS.find(
          (item) => item.id === resolvedSectionId,
        );
    ...
    switch (entityType) {
      ...
      case "artsCraft": {
        ...
        if (parsedIndex === undefined) {
          await Promise.all(
            images.map((url) => maybeDeleteCloudinaryAsset(url)),
          );
          record = await prisma.artsCraftProject.update({
            where: { id: entityId },
            data: { images: [] },
      ...
      case "navbarMenu": {
        ...
        if (parsedIndex === undefined) {
          await Promise.all(
            nextImages.map((url) => maybeDeleteCloudinaryAsset(url)),
          );
          const saved = await writeNavbarMenuImages({
            ...navbarState.sections,
